...
class Arbitrator(threading.Thread):
    ...
    def __setup(self):
        ...
        if DB_SOURCE == 'sqlite':
            ...
            self.dbcur = self.dbcon.cursor()
            self.dbcur.execute("CREATE TABLE IF NOT EXISTS synced_files (input_file VARCHAR(2048), transported_file_basename VARCHAR(2048), url VARCHAR(2048), server VARCHAR(255), UNIQUE INDEX file_unique_per_server (input_file(128), server()))")
        else:
            self.logger.error("Invalid DB_SOURCE detected")
